...
namespace GeneralStore.Controllers
{
    public static class SessionController
    {
        ...
        {            
            ...
            app.MapGet("/api/session/{id}", async (GeneralStore.Interfaces.ISession repo, string id) => await repo.GetAsync(id));
            app.MapGet("/api/session/last", async (GeneralStore.Interfaces.ISession repo) => await repo.GetLastAsync());

            app.MapPost("/api/session/createSessionIfNotExists/{id}", async (Interfaces.ISession repo, string id) =>
            {
                var newItem = await repo.CreateAsync(id);
                return Results.Created($"/record/{newItem.Id}", newItem);
            });

            app.MapPut("/api/session/{id}", async (GeneralStore.Interfaces.ISession repo, Session updaterecord, string id) =>
            {
            ...
            app.MapDelete("/api/session/{id}", async (GeneralStore.Interfaces.ISession repo, string id) =>
            {
